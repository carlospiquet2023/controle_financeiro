...
export default async function Home({ searchParams }: { searchParams: Promise<{ month?: string; page?: string }> }) {
  ...
  const transactionInclude = { card: true, category: true, responsiblePerson: true, sharedComments: { orderBy: { createdAt: "desc" as const }, take: 3 } } as const;
  const [household, accounts, cards, categories, people, monthTransactions, overviewTransactions, expenseSummary, futureTransactions, openSplits, imports, sharedLinks] = await Promise.all([
    ...
    db.sharedLedgerLink.findMany({ where: { householdId, month: { gte: month, lt: nextMonth }, active: true }, select: { id: true, createdAt: true }, orderBy: { createdAt: "desc" } }),
  ]);
